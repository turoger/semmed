...
# This script will generate a dataset that removes all "NEG_" edges and replaces them with bi-directional edges.
import os, pickle
...
# seperate by column
edges0 = edges.query('Neg==0')
edges1 = edges.query('Neg==1')
# create a new column 'type1' with removed 'NEG_'
print('Removing Negative Edges and Replacing with bidirectional where possible')
edges1['type1']=edges1['type'].apply(lambda x: x[4:])
edges1 = edges1.drop(columns = 'type').rename(columns = {'type1':'type'})
edges = pd.concat([edges0,edges1])
