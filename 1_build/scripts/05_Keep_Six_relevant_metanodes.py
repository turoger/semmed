...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        ...
        usage='03_Condense_edge_semmantics.py [<args>] [-h | --help]'
    )
...
def main(args):
    ...
    nodes = gt.remove_colons(pd.read_csv(f'../data/nodes_{args.semmed_version}_consolidated_condensed_filtered_001.csv'))
    edges = gt.remove_colons(pd.read_csv(f'../data/edges_{args.semmed_version}_consolidated_condensed_filtered_001.csv'))
    ...
    gt.add_colons(nodes).to_csv(f'../data/nodes_{args.semmed_version}_cons_6_metanode.csv', index=False)
    gt.add_colons(edges).to_csv(f'../data/edges_{args.semmed_version}_cons_6_metanode.csv', index=False)
